...
export default async function handler(req,res){
  ...
  if(!paymentId||!txid) return json(res,400,{ok:false,error:"missing_payment_data"});
  try{
    const r=await fetch(`${API_BASE}/payments/${encodeURIComponent(paymentId)}/complete`,{method:"POST",headers:{Authorization:`Key ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({txid})});
    const text=await r.text(); let data={}; try{data=JSON.parse(text)}catch{}
    if(!r.ok) return json(res,r.status,{ok:false,error:"pi_completion_failed",message:data?.error||data?.message||text||"Pi completion failed",pi:data});
    return json(res,200,{ok:true,payment:data,txid});
